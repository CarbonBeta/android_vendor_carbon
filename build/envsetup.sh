...
- losremote:       Add git remote for LineageOS Gerrit Review.
...
function losremote()
{
    ...
    git remote rm carbon 2> /dev/null
    ...
    local LINEAGE_USER=$(git config --get review.review.carbonos.org.username)
    if [ -z "$LINEAGE_USER" ]
    then
        git remote add lineage ssh://review.lineageos.org:29418/$PFX$PROJECT
    else
        git remote add lineage ssh://$CARBON_USER@review.lineageos.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'carbon' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/CarbonROM/$PROJECT
...
function makerecipe() {
    ...
    then
        pwd
        carbonremote
        git push carbon HEAD:refs/heads/'$1'
    fi
    '
}
